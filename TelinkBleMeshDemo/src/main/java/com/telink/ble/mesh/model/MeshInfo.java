...
public class MeshInfo implements Serializable, Cloneable {
     ...
     */
    public List<DeviceInfo> nodes = new ArrayList<>();
...
//    public int netKeyIndex;
    ...
    public List<TimerInfo> globalTimers = new ArrayList<>();

    public boolean removeGroupByGroupAddress(int address) {
        Iterator<GroupInfo> iterator = groups.iterator();
        while (iterator.hasNext()) {
            GroupInfo groupInfo = iterator.next();

            if (groupInfo.address == address) {
                iterator.remove();
                return true;
            }
        }
    ...
    private boolean getTimerById(int id) {
        Iterator<TimerInfo> iterator = globalTimers.iterator();
        while(iterator.hasNext()) {
    ...
    public void sortTimers() {
        Collections.sort(globalTimers);
    }

    public void removeTimerByPosition(int position) {
        globalTimers.remove(position);
    }
     ...
     */
    public DeviceInfo getDeviceByUUID(@NonNull byte[] deviceUUID) {
        for (DeviceInfo info : nodes) {
    ...
    public void insertDevice(DeviceInfo deviceInfo) {
        DeviceInfo local = getDeviceByUUID(deviceInfo.deviceUUID);
        if (local != null) {
    ...
    public boolean removeDeviceByMeshAddress(int address) {
        ...
        Iterator<DeviceInfo> iterator = nodes.iterator();
        while (iterator.hasNext()) {
