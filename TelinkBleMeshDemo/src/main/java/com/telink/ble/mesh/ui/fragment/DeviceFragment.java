...
//                    startActivity(new Intent(getActivity(), DeviceProvisionActivity.class));
        ...
        view.findViewById(R.id.tv_sync_time).setOnClickListener(this);
        view.findViewById(R.id.tv_set_timer).setOnClickListener(this);
...
//                startTime = System.currentTimeMillis();
                ...
                MeshService.getInstance().sendMeshMessage(onOffSetMessage);
                TransMeshMessage.getInstance().SetOnOff(0xFFFF, 1);
                break;
            case R.id.tv_off:
//                startTime = System.currentTimeMillis();
                ...
                MeshService.getInstance().sendMeshMessage(onOffSetMessage);
                TransMeshMessage.getInstance().SetOnOff(0xFFFF, 0);
                break;
            case R.id.tv_sync_time:
                showWaitingDialog("setting");
                TransMeshMessage.getInstance().SyncDeviceSystemTime(0xFFFF);
                mCycleHandler.postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        // on grouping timeout
