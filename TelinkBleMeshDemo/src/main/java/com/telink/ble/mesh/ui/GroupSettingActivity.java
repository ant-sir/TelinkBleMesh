...
public class GroupSettingActivity extends BaseActivity implements EventListener<String> {
        ...
        private void onProgressUpdate(SeekBar seekBar, int progress, boolean immediate) {
            ...
            if ((currentTime - this.preTime) >= DELAY_TIME || immediate) {
                ...
                if (seekBar == lum) {
                    ...
                    TransMeshMessage.getInstance().SetGroupLum(group.address, progress);
                    ...
                    meshMessage = CtlTemperatureSetMessage.getSimple(group.address,
                            ...
                            0, false, 0);
                    MeshService.getInstance().sendMeshMessage(meshMessage);
                    TransMeshMessage.getInstance().SetGroupTemp(group.address, progress);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        findViewById(R.id.tv_delete).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                TelinkMeshApplication.getInstance().getMeshInfo().removeGroupByGroupAddress(group.address);
                TelinkMeshApplication.getInstance().getMeshInfo().saveOrUpdate(getApplicationContext());
                TelinkMeshApplication.getInstance().dispatchEvent(new GroupStatusChangedEvent(this, GroupStatusChangedEvent.EVENT_TYPE_GROUP_STATUS_CHANGED, group));
                finish();
            }
        });
    ...
    private List<DeviceInfo> getDevicesInGroup() {
        ...
        List<DeviceInfo> localDevices = TelinkMeshApplication.getInstance().getMeshInfo().nodes;
