    ...
    private void setNextModel() {
        if (modelIndex > models.length - 1) {
            if (opType == 0) {
                ...
                TransMeshMessage.getInstance().SetGroup(address, opGroupAdr, 1);
                MeshLogger.log(String.valueOf(address) + " add " + String.valueOf(opGroupAdr), "zyl", MeshLogger.LEVEL_INFO);
            } else {
                deviceInfo.subList.remove((Integer) opGroupAdr);
                TransMeshMessage.getInstance().SetGroup(address, opGroupAdr, 0);
                MeshLogger.log(String.valueOf(address) + " remove " + String.valueOf(opGroupAdr), "zyl", MeshLogger.LEVEL_INFO);
            }
